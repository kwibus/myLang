...
close :: MonoType -> PolyType
close t = undefined -- fst3 $ go t fEmtyEnv 0
...
solver :: BruijnTerm i -> Either (TypeError i) PolyType
...
solveWith :: BruijnTerm i -> FreeEnv MonoType -> BruiEnv Free ->
    Infer i (MonoType, FreeEnv MonoType )
...
solveWith (Val _ v) env _ = return (btype v, env)
...
unifyEnv :: FreeEnv MonoType -> FreeEnv MonoType -> Either [UnificationError i] (FreeEnv MonoType)
unifyEnv env1 env2 = IM.foldWithKey f (Right env1) env2
    where f key typ1 (Right env) = case IM.lookup key env of
            Nothing -> mapLeft (: []) $ unify typ1 (TVar key) env
...
unify :: MonoType -> MonoType -> FreeEnv MonoType ->
    Either (UnificationError i) (FreeEnv MonoType) -- retunr type
...
apply :: MonoType -> FreeEnv MonoType -> MonoType
...
bind :: Free -> MonoType -> FreeEnv MonoType ->
     Either (UnificationError i) (FreeEnv MonoType)
bind n1 t env
...
infinit :: Free -> MonoType -> FreeEnv MonoType -> Bool
...
isIn :: Free -> MonoType -> FreeEnv MonoType -> Bool
isIn _ TVal {} _ = False
...
unifys :: MonoType -> MonoType -> FreeEnv MonoType -> Bool
